...
def main() -> None:
    ...
    assert len(rows) == 327
    assert sum(len(ledger.sorted_q_vectors(10 - a)) for a in range(11)) == 327

    for row in rows:
        a = row["a_t"]
        b = tuple(row["b"])
        assert tuple(sorted(b, reverse=True)) == b
        assert a + sum(b) + row["residual_degree_budget"] == 10
        for branch in ("T1", "T2"):
            exact = ledger.terminal_test(a, b, branch=branch)
            brute = brute_terminal(a, b, branch)
            assert exact.feasible == brute, (a, b, branch, exact, brute)
            assert all(
