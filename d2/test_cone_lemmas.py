...
def main() -> None:
    ...
    for record in certs["branches"]:
        ...
        if record["certified_kill"]:
            kills += 1
            assert not open_cases, key
            assert engine_status[key] == "engine_killed_pending_audit", key
        else:
            assert open_cases, key
            assert engine_status[key] == "survives", key
    assert kills == 390, kills
    assert certs["summary"]["kills_certified"] == 390
    ...
    for record in sample:
        ...
        stored = record["cases"]
        assert len(regenerated) == len(stored)
        for fresh, kept in zip(regenerated, stored):
            if fresh is None:
                assert kept["kind"] == "open"
            else:
                assert fresh["kind"] == kept["kind"]
                if fresh["kind"] == "B":
                    assert fresh["dimension"] == kept["dimension"]
                    assert fresh["sum_of_minima"] == kept["sum_of_minima"]
    ...
    for entry in unconditional:
        assert entry["beta"] not in live_betas or (
