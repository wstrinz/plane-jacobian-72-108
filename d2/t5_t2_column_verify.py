...
def load_cells() -> dict[tuple[int, tuple[int, ...]], dict]:
    ...
    assert len(cells) == 12 and all(key in cells for key in TARGETS)
    assert sum(row["survivor_case_count"] for row in cells.values()) == 32
    for key, row in cells.items():
        assert row["status"] == "survives"
        assert row["survivor_case_count"] == len(row["survivor_cases"])
        assert tuple(case_flag(case) for case in row["survivor_cases"]) == EXPECTED_FLAGS[key]
        assert all(not case["sigma_zero"] for case in row["survivor_cases"])
        assert all(len(case["witness"]) == 5 for case in row["survivor_cases"])
    return cells
...
def check_tropical_line(left: int | None, outer: int | None, horders: list[int]) -> None:
    ...
    if outer is None:
        assert (left == hmin) if not htied else (left is None or left >= hmin)
    elif outer < hmin:
        assert left == outer
    elif outer > hmin and not htied:
        assert left == hmin
    else:
        assert left is None or left >= min(outer, hmin)
...
def verify_witnesses(cells: dict) -> None:
    for (a, bexpected), row in cells.items():
        ...
        for case in row["survivor_cases"]:
            ...
            assert b == bexpected
            assert all(3 * bi + mi == 6 + 2 * si for bi, si, mi in zip(b, s, m))
            tp = next(place for place in case["witness"] if place["place"] == "t")
            assert int(tp["b"]) == a and int(tp["v_sigma"]) == 0
            assert int(tp["v_g"]["6"]) == 0
            vd2 = finite(tp["v_d2"])
...
def invariants(row: dict) -> tuple[int, int, int, int, int, int]:
    ...
    assert all(q_profile(case) == (b, s, m) for case in row["survivor_cases"])
...
def main() -> None:
    ...
    assert sp.expand(h[5].subs(d1, 0) - (-9216 * d2 * sigma**2 + 2048 * e**2)) == 0
    h4 = -5184 * d2**2 * sigma**2 + 5632 * d2 * e**2 - 12096 * sigma**3
    assert sp.expand(h[4].subs(d1, 0) - h4) == 0
    assert sp.expand(h[4].subs({d1: 0, d2: 0}) + 12096 * sigma.subs(d2, 0)**3) == 0
    assert sp.degree(Phi, y) == 34
    ...
    assert sp.cancel(lhs - rhs - wanted) == 0
    ...
    assert Hexpr[7] == 0 and sp.expand(Hexpr[6] + 3072 * s**2) == 0
    assert sp.expand(Hexpr[5] - (-9216 * d2 * s**2 + 2048 * e**2)) == 0
    ...
    assert tuple(top - other for _, _, _, other, top in margins) == (8, 3, 5, 4, 2, 4, 1)
