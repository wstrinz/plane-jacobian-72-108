...
def check_local_shape() -> None:
    ...
    assert {(h, d, r) for h, d, r, _z, _k, _r5 in survivors} == {(5, 4, 2)}
    assert all(z >= 3 for _h, _d, _r, z, _k, _r5 in survivors)
...
def infinity_degrees(d: int, z: int | None) -> tuple[list[int], tuple[int, ...]]:
    ...
    if tops == (5,):
        assert h5_terms.count(max(h5_terms)) == 1
    elif tops == (6,):
        assert h6_terms.count(max(h6_terms)) == 1
    return degrees, tops
...
def check_infinity_table() -> None:
    ...
    assert sum(count for tops, count in counts.items() if len(tops) == 1) == 43
    assert ties == [(2, z, (5, 6, 7)) for z in [None, *range(6)]]
...
def check_square_constraints() -> None:
    ...
    assert sp.factor(multiplier*local_level6 - local_square) == 0
    ...
    assert sp.factor(-gamma**3*infinity_level6/c**5 - infinity_square) == 0
...
def check_tail_refinement(h: dict[int, sp.Expr]) -> None:
    ...
    assert sp.expand(tail - square - residual) == 0

    assert 6*34 + 3*9 + 2*5 == 241
    assert 5*34 + 2*35 == 240
    residual_inner_bounds = [
        2*4 + 34, 2*4 + 4 + 27, 4 + 4 + 27,
        34 + 4 + 4, 4 + 8 + 27, 2 + 4 + 36,
    ]
    assert max(residual_inner_bounds) == 42
    assert 5*34 + 3*9 + 42 == 239
    ...
    assert max(h4_bound_terms) == 22
    assert 4*34 + 9*9 + 22 == 239
    assert h4_sigma != 0
    ...
    poly = sp.Poly(core, y)
    assert sp.factor(poly.coeff_monomial(y**6)) == 0
    expected_y5 = gamma**4*(sp.Rational(25, 4) - v1)
    assert sp.factor(poly.coeff_monomial(y**5) - expected_y5) == 0
    ...
    assert sp.factor(degree239) == -12884901888*c**2*sigma4**2
...
def main() -> None:
    ...
    assert sp.expand(h[6] - (-3072*sigma**2 + 14336*d1**2*d2 + 8192*d1*e)) == 0
    assert sp.expand(
