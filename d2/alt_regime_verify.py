...
assert Ehat.subs(y, -1) != 0 and sp.degree(Ehat, y) <= 15-a
Efull = t**a*Ehat
...
hh[0] = -U*r[0]/E**21
assert sp.cancel(sum(T**(7-f)*U**f*E**(21-3*f)*hh[f] for f in range(8))) == 0
...
assert sp.expand(terminal-generic) == 0
...
assert sp.expand(hs[6]-(-3072*sigma**2+14336*d1**2*d2+8192*d1*dm1)) == 0
assert sp.expand(hs[5].subs(d1,0)-(-9216*d2*sigma**2+2048*dm1**2)) == 0
...
def t1_order(s, m):
    ...
    if s % 2:
        for x in range(anchor, s):
            k = 2*x-s
            assert 0 <= k < s and k%2 and 2*x > k and x+m > k
            assert all(2*z != k for z in range(30))
        return s
    return anchor
...
assert {a:t1_order(3*a-30,a) for a in range(11,16)} == {11:3,12:3,13:9,14:6,15:15}
assert {b:t1_order(3*b-1,b) for b in range(1,5)} == {1:1,2:5,3:4,4:11}
assert {a:t2_order(3*a-30,a) for a in range(11,16)} == {11:("order",3),12:("order",6),13:("order",9),14:("order",12),15:("order",15)}
assert {b:t2_order(3*b-1,b) for b in range(1,5)} == {1:("order",2),2:("killed",None),3:("order",7),4:("killed",None)}
...
assert len(rows) == 26 and all(row["stratum_status"] == "alternate_regime_open" for row in rows)
...
def audit(a, b, branch):
    ...
    al=tord+sum(qord)
    qg=tuple(level+2*x-3*bi for x,bi in zip(qord,b)); assert all(x>=0 for x in qg)
    gl=2*tord+sum(qg); gc=raw-3*B
    return {"killed":local or al>ac or gl>gc,"aux":al,"g":gl,"gcap":gc}

V={}
for row in rows:
    a,b=row["a_t"],tuple(row["b"]); assert a+sum(b)<=15
...
assert K1==[(11,(2,1,1,0)),(11,(2,2,0,0)),(11,(4,0,0,0)),(13,(1,0,0,0)),(13,(1,1,0,0)),(13,(2,0,0,0)),(15,(0,0,0,0))]
assert K2==[(11,(2,0,0,0)),(11,(2,1,0,0)),(11,(2,1,1,0)),(11,(2,2,0,0)),(11,(4,0,0,0)),(12,(2,0,0,0)),(12,(2,1,0,0)),(12,(3,0,0,0)),(13,(1,1,0,0)),(13,(2,0,0,0)),(14,(1,0,0,0)),(15,(0,0,0,0))]
assert sum(z["killed"] for z in V.values())==19
assert sum(not z["killed"] for z in V.values())==33
