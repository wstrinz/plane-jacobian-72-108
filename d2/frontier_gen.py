...
# ---------------------------------------------------------------------------
# Deterministic provenance.  The old build embedded the wall clock and each
# artifact's file mtime, so FRONTIER.md could not be regenerated byte-for-byte
# from a clean clone.  We content-address each source artifact by SHA-256 and
# stamp the generating git commit + schema instead; SOURCE_DATE_EPOCH supplies
# an optional reproducible UTC build date.
# ---------------------------------------------------------------------------
SCHEMA = 1
...
def _source_date():
    ...
    if epoch:
        return datetime.datetime.fromtimestamp(
            int(epoch), tz=datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    return None
...
def load(name, required=True):
    ...
    # Content address (mtime-independent) so the output is reproducible.
    return data, _file_sha(path)
...
def main():
    sub2_d, sub2_h = load(SUB2_ART)
    sub1_d, sub1_h = load(SUB1_ART)
    alt_d, alt_h = load(ALT_ART)
    pd2_d, pd2_h = load(PHASE_D_SUB2, required=False)
    pd1_d, pd1_h = load(PHASE_D_SUB1, required=False)
    ...
    lines.append("")
    _sdate = _source_date()
    lines.append("Provenance: git %s, schema %d.%s" % (
        _git_commit(), SCHEMA,
        "" if _sdate is None else " Built %s." % _sdate))
    lines.append("")
    lines.append("Source-artifact SHA-256 (first 16 hex; content-addressed, mtime-independent):")
    lines.append("")
    lines.append("| Artifact | SHA-256 (16) | partial_checkpoint |")
    lines.append("|---|---|---|")
    lines.append("| `%s` | %s | %s |" % (SUB2_ART, sub2_h[:16],
                                         sub2_d.get("partial_checkpoint")))
    lines.append("| `%s` | %s | %s |" % (SUB1_ART, sub1_h[:16],
                                         sub1_d.get("partial_checkpoint")))
    lines.append("| `%s` | %s | %s |" % (ALT_ART, alt_h[:16],
                                         alt_d.get("partial_checkpoint")))
    if pd2:
        lines.append("| `%s` | %s | (worklist) |" % (PHASE_D_SUB2, pd2_h[:16]))
    if pd1:
        lines.append("| `%s` | %s | (worklist) |" % (PHASE_D_SUB1, pd1_h[:16]))
    lines.append("")
