...
assert sp.expand(H[6] - (14336*d1**2*d2 + 8192*d1*e - 3072*sigma**2)) == 0
assert sp.expand(H[5] - (-12288*d1**2*d2**2 + 32256*d1**2*sigma
                         + 18432*d1*d2*e - 9216*d2*sigma**2 + 2048*e**2)) == 0
...
hh[0] = -U*r[0]/E**21
assert sp.cancel(sum(A**(7-f)*U**f*E**(21-3*f)*hh[f] for f in range(8))) == 0
for b in range(1, 5):
    s = 3*b-1
    assert all(f+b*(21-3*f) == 7+(7-f)*s for f in range(8))
T = sp.symbols("T", nonzero=True); X = list(sp.symbols("X0:8")); g = {1: T*X[0]}
for level in range(1, 7):
    g[level+1] = sp.expand(T*(E**3*g[level] + U**level*X[level]))
assert sp.expand(E**3*g[7] + U**7*X[7]
                 - sum(T**(7-f)*U**f*E**(21-3*f)*X[f] for f in range(8))) == 0
...
assert [m for m, _ in MONS[6]] == [(2,1,0,0), (1,0,0,1), (0,0,2,0)]
assert {m for m, _ in MONS[5]} == {
    (2,2,0,0), (2,0,1,0), (1,1,0,1), (0,1,2,0), (0,0,0,2)}
INF, BOUND = 1000, 100
...
assert T1F == {
 ...
 "q3":{(x,x-4) for x in range(4,10)}, "q4":set()}
assert T1Z == {"t11":set(range(5,10)), "t12":{9}, "t13":set(), "t14":set(),
                "q1":set(range(3,10)), "q2":{7}, "q3":set(), "q4":set()}
T2 = {p:{z for z in range(13) if any(t2_local(s,m,z,k) for k in KS)}
      for p,(s,m) in PLACES.items()}
assert T2 == {"t11":set(range(3,13)), "t12":set(range(6,13)),
...
def oldkill(a,b,branch):
    ...
    gos=tuple(lev+2*x-3*bi for x,bi in zip(qord,b)); assert all(x>=0 for x in gos)
...
assert newkills==expected and len(survivors)==27
...
assert sp.expand(3*(aa+2*uu)+(aa+2*vv)-2*(2*aa+3*uu+vv))==0
for a,b,br in survivors:
    assert 15-a-sum(b)>=0
    if br=="T1": assert any(global_states(a,b))
    else: assert min(T2[f"t{a}"])+sum(min(T2[f"q{x}"]) for x in b if x)<=12
